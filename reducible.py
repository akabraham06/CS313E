...
def step_size(s):
    ...
    """
    # step_size = constant - (key_value % constant)
...
def insert_word(s, hash_table):
    """
    Inserts a string into the hash table using open addressing.
...
def get_longest_words(string_list):
    ...
    for i, _ in enumerate(string_list):
        ...
        for j in range(i + 1, len(string_list)):
            if string_list[j] < string_list[mi]:
                mi = j
        temp = string_list[i]
        string_list[i] = string_list[mi]
        string_list[mi] = temp
    max_len = 0
    ...
    for i in range(len(string_list)):
        if len(string_list[i]) == max_len:
            biggest_words.append(string_list[i])
...
def main():
    ...
    for _, s in enumerate(word_list):
        if is_reducible(s, hash_list, hash_memo):
            reducible_words.append(s)
    a = get_longest_words(reducible_words)
